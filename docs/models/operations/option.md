...
```go
provesdkservergo.WithServerURL("https://api.example.com")
```
...
```go
provesdkservergo.WithTemplatedServerURL("https://{host}:{port}", map[string]string{
...
```go
provesdkservergo.WithServer("my-server")
```
...
```go
provesdkservergo.WithClient(httpClient)
```
...
```go
provesdkservergo.WithSecuritySource(/* ... */)
```
...
```go
provesdkservergo.WithRetryConfig(retry.Config{
    Strategy: "backoff",
...
```go
provesdkservergo.WithTimeout(30 * time.Second)
```
