// Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.

package components

type V3ChallengeAddressEntryRequest struct {
	// The street address of the individual.
	Address *string `json:"address,omitempty"`
	// The city of the individual.
	City *string `json:"city,omitempty"`
	// The apartment number or other extended address information.
	ExtendedAddress *string `json:"extendedAddress,omitempty"`
	// The zip code of the individual. It can be either 5 digits (XXXXX) or ZIP+4 (XXXXX-XXXX).
	PostalCode *string `json:"postalCode,omitempty"`
	// The state or locality of the individual.
	Region *string `json:"region,omitempty"`
}

func (v *V3ChallengeAddressEntryRequest) GetAddress() *string {
	if v == nil {
		return nil
	}
	return v.Address
}

func (v *V3ChallengeAddressEntryRequest) GetCity() *string {
	if v == nil {
		return nil
	}
	return v.City
}

func (v *V3ChallengeAddressEntryRequest) GetExtendedAddress() *string {
	if v == nil {
		return nil
	}
	return v.ExtendedAddress
}

func (v *V3ChallengeAddressEntryRequest) GetPostalCode() *string {
	if v == nil {
		return nil
	}
	return v.PostalCode
}

func (v *V3ChallengeAddressEntryRequest) GetRegion() *string {
	if v == nil {
		return nil
	}
	return v.Region
}
